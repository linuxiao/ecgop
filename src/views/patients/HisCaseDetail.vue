# Create new file
<template>
  <div class="his-case-detail">
    <el-card class="detail-card">
      <template #header>
        <div class="card-header">
          <span class="title">HIS病例详情</span>
          <div class="actions">
      ...
      <el-descriptions title="基本信息" :column="3" border>
        <el-descriptions-item label="开单号">{{ caseDetail.orderNo }}</el-descriptions-item>
        <el-descriptions-item label="开单时间">{{ caseDetail.orderTime }}</el-descriptions-item>
        ...
        <el-descriptions-item label="患者编号">
          <el-link type="primary" @click="handleViewPatient">{{ caseDetail.patientNo }}</el-link>
        </el-descriptions-item>
        <el-descriptions-item label="患者姓名">{{ caseDetail.patientName }}</el-descriptions-item>
        <el-descriptions-item label="性别">{{ caseDetail.gender }}</el-descriptions-item>
        <el-descriptions-item label="年龄">{{ caseDetail.age }}岁</el-descriptions-item>
        <el-descriptions-item label="联系电话">{{ caseDetail.phone }}</el-descriptions-item>
        <el-descriptions-item label="医保类型">{{ caseDetail.insuranceType }}</el-descriptions-item>
      </el-descriptions>

      <!-- 就诊信息 -->
      <el-descriptions title="就诊信息" :column="2" border class="mt-16">
        <el-descriptions-item label="就诊科室">{{ caseDetail.department }}</el-descriptions-item>
        <el-descriptions-item label="主治医生">{{ caseDetail.doctor }}</el-descriptions-item>
        <el-descriptions-item label="主诉" :span="2">{{ caseDetail.complaint }}</el-descriptions-item>
        <el-descriptions-item label="现病史" :span="2">{{ caseDetail.presentIllness }}</el-descriptions-item>
        <el-descriptions-item label="既往史" :span="2">{{ caseDetail.pastHistory }}</el-descriptions-item>
        <el-descriptions-item label="诊断结果" :span="2">{{ caseDetail.diagnosis }}</el-descriptions-item>
        <el-descriptions-item label="处理意见" :span="2">{{ caseDetail.treatment }}</el-descriptions-item>
        <el-descriptions-item label="医嘱" :span="2">{{ caseDetail.advice }}</el-descriptions-item>
      </el-descriptions>
...
const handleViewPatient = () => {
  router.push(`/patients/${caseDetail.value.patientNo}`)
}
...
const loadCaseDetail = async () => {
  // TODO: 从后端加载病例详情数据
  console.log('Loading case detail for:', caseId)
}
